...
const (
	...
	MessageResourceExists = "Resource %q already exists and is not managed by CustomDeployment"
	// MessageResourceSynced is the message used for an Event fired when a Foo
	// is synced successfully
	MessageResourceSynced = "CustomDeployment synced successfully"
)
...
func NewController(
	...
	customDeploymentInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: controller.enqueueCustomDeployment,
		UpdateFunc: func(old, new interface{}) {
			controller.enqueueCustomDeployment(new)
		},
	})
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	klog.Info("Starting CustomDeployment controller")
...
func (c *Controller) syncHandler(key string) error {
	...
	cd, err := c.customDeploymentsLister.CustomDeployments(namespace).Get(name)
	if err != nil {
		// The Foo resource may no longer exist, in which case we stop
		// processing.
		if errors.IsNotFound(err) {
			utilruntime.HandleError(fmt.Errorf("customDeployment '%s' in work queue no longer exists", key))
			return nil
		}
	...
	c.recorder.Event(cd, corev1.EventTypeNormal, SuccessSynced, MessageResourceSynced)
	return nil
}
...
func (c *Controller) updateCustomDeploymentStatus(cd *samplev1alpha1.CustomDeployment, deployment *appsv1.Deployment) error {
	...
	customDeploymentCopy := cd.DeepCopy()
	// If the CustomResourceSubresources feature gate is not enabled,
	// we must use Update instead of UpdateStatus to update the Status block of the Foo resource.
	// UpdateStatus will not allow changes to the Spec of the resource,
	// which is ideal for ensuring nothing other than resource status has been updated.
	_, err := c.sampleclientset.SamplecontrollerV1alpha1().CustomDeployments(cd.Namespace).Update(context.TODO(), customDeploymentCopy, metav1.UpdateOptions{})
	return err
}
...
func (c *Controller) enqueueCustomDeployment(obj interface{}) {
	var key string
	var err error
...
func (c *Controller) handleObject(obj interface{}) {
	...
	if ownerRef := metav1.GetControllerOf(object); ownerRef != nil {
		...
		// with it.
		if ownerRef.Kind != "CustomDeployment" {
			return
		}

		cd, err := c.customDeploymentsLister.CustomDeployments(object.GetNamespace()).Get(ownerRef.Name)
		if err != nil {
			klog.V(4).Infof("ignoring orphaned object '%s' of customdeployment '%s'", object.GetSelfLink(), ownerRef.Name)
			return
		}

		c.enqueueCustomDeployment(cd)
		return
	}
}
